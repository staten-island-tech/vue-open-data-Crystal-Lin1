<template>
    <div class="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-10">
        <div class="shadow-lg w-[50%] p-8 m-auto rounded-2xl border border-gray-100 bg-white max-h-[80vh] overflow-y-auto">
        ...
        class="text-sm text-gray-600 mt-2 p-3 bg-gray-50 rounded-xl border border-gray-100"
        >
        {{ selectedExplanation }}
        </p>
...
const parameters = [
  {
    name: 'CAMIS',
    explanation: 'Unique identifier for the restaurant',
    moreInfo: 'Always a 10 digit number',
  },
  {
    name: 'DBA',
    explanation: 'Restaurant name',
    moreInfo: 'The name of the restaurant as it appears to consumers',
  },
  {
    name: 'BUILDING',
    explanation: 'Building number',
    moreInfo: 'Just the number part of the address, no street name or anything else',
  },
  {
    name: 'ACTION',
    explanation: 'Consequences of the inspection',
    moreInfo: 'Examples include nothing or closing',
  },
  {
    name: 'CRITICAL FLAG',
    explanation: 'Critical, N/A, or Not Critical',
    moreInfo: 'Indicates whether a violation is critical, not critical, or not applicable',
  },
...
const moreInfo = computed(() => parameters.find((p) => p.name === selected.value)?.moreInfo ?? '')
</script>
